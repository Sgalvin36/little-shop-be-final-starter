...
describe "Merchant endpoints", :type => :request do
    describe "Get all merchants" do
        ...
        it "should return a coupon_count attribute and invoice_coupon_count attribute" do
            customer = create(:customer)
            merchant1 = create(:merchant)
            merchant2 = create(:merchant)
            ...
            create_list(:coupon, 3, merchant_id: merchant1.id)
            create(:invoice, status: "returned", customer_id: customer.id, merchant_id: merchant1.id, coupon_id: coupon1.id)
            create_list(:invoice, 3, status: "shipped", customer_id: customer.id, merchant_id: merchant1.id)

            create(:invoice, status: "packaged", customer_id: customer.id, merchant_id: merchant2.id, coupon_id: coupon2.id)
            create(:invoice, status: "shipped", customer_id: customer.id, merchant_id: merchant2.id, coupon_id: coupon2.id)
            
            get api_v1_merchants_path
            expect(response).to be_successful

            data = JSON.parse(response.body, symbolize_names: true)
            data[:data].each do |merchant|
                expect(merchant[:type]).to eq("merchant")
                expect(merchant[:attributes][:name]).to be_a(String)
                expect(merchant[:attributes][:coupon_count]).to be_a(Integer)
